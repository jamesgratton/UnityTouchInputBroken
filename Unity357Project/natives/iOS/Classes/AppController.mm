...
- (void) registerAccelerometer
{
	...
	// if CoreMotion API is available lets use it for accelerometer updates
	// otherwise old accelerometer API affects gyro accuracy
	if (sMotionManager != nil)
	{
		if (frequency > 0)
		{
			...
			[sMotionManager startAccelerometerUpdatesToQueue: sMotionQueue withHandler: ^( CMAccelerometerData* data, NSError* error) {
				UnityDidAccelerate(data.acceleration.x, data.acceleration.y, data.acceleration.z, data.timestamp);
			}];
...
- (void) Repaint
{
    ...
    if (UnityGetAccelerometerFrequency() > 0 && !_accelerometerIsActive)
    {
        static int frameCounter = 90;
        if (frameCounter <= 0)
        {
            // NOTE: work-around for accelerometer sometimes failing to register (presumably on older devices)
